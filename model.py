...
class DvectorModel(nn.Module):
    def __init__(
        ...
        self.linear1 = nn.Linear(n_mels*401,embedding_size*64)
        self.linear2 = nn.Linear(embedding_size*32, embedding_size*16)
        self.linear3 = nn.Linear(embedding_size*8, embedding_size*4)
        self.linear4 = nn.Linear(embedding_size*2, embedding_size*2)
        self.linear5 = nn.Linear(embedding_size, class_size)
        self.batchnorm1 = nn.BatchNorm1d(embedding_size*64)
        self.batchnorm2 = nn.BatchNorm1d(embedding_size*16)
